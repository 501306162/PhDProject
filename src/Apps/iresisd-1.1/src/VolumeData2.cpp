...
void VolumeData::loadData()
{
	...
	try
	{
		...
		for(lit = selectedSeries.begin(); lit != lend; ++lit)
		{
			...
			cout << seriesNumber[*lit].front() << "-" << seriesNbSlices[*lit] << endl;
		}
...
void VolumeData::printListedSeries()
{
	...
	for (int n=0; n<listeSeriesSize; n++)
	{
		...
		cout << "Series numbers: " << seriesNumber[n].front() << std::endl;
		cout << endl;
	}
}
...
void VolumeData::handle_similar_series(set<int>::iterator& it_serie, set<int>::iterator& it_serie_add)
{
	...
	printImage(*it_serie, 0, 0, "Serie #1");
	myargs userData1;
	userData1.ptr = this;
	...
	printImage(*it_serie_add, 0, 0, "Serie #2");
	myargs userData2;
	userData2.ptr = this;
	...
	valIni = (int)maxVal;
	cv::createTrackbar("range", "Serie #2", &valIni, valIni, updateRange, &userData2);
...
void VolumeData::fuse_series(set<int>::iterator& it_serie, set<int>::iterator& it_serie_add)
{
	...
	for (int s=0; sit != send; ++sit, s++)
	{
		...
		{
			if ( fabs(sitdists->first - *sit) < 1e-5 )
			{
				found = true;
		...
		else // else, let the user choose which slice to use
		{
			...
			printImage(*it_serie_add, s, 0, "Choice #1");
			myargs userData1;
			userData1.ptr = this;
			...
			printImage(conflictingSerie, conflictingSlice, 0, "Choice #2");
			myargs userData2;
			userData2.ptr = this;
			...
			cv::createTrackbar("trackbar2", "Choice #2", &valIni, valIni, updateRange, &userData2);
			cv::waitKey(100);
			...
			while(!stop)
			{
				...
				switch(choice)
				{
					case 1:
						...
						dists.erase( sitdists );
						dists.insert( make_pair(*sit, make_pair(*it_serie_add, s) ) );
...
void VolumeData::printImage(int numSerie, int slice, int timeframe, string win_name, int range)
{
	...
	for(int c=0; c<Columns; c++)
	{
		imgDisp.at<double>(r, c) = (double)seriesDataValues[numSerie][slice][timeframe][c + Columns * r];
		
		if (imgDisp.at<double>(r, c) > max) max = imgDisp.at<double>(r, c);
	}
	...
	for(; lit != lend; ++lit)
	{
		...
		if (status.good())
		{
			...
			if (!seriesDescription.empty())
			{
				...
				for (int n=0; n<listeSeriesSize; n++)
				{
					...
					{						
						...
						seriesNumber[n].push_back(SeriesNumber);
			...
			if (!knownSerie)
			{
				...
				newVec.push_back(SeriesNumber);
				seriesNumber.push_back(newVec);
...
void VolumeData_dicom::printSelectedSeries()
{
	...
	for (; lit != lend; ++lit)
	{
		...
		cout << endl;
		//for(int i = 0; i < seriesNumber[*lit].size(); i++) cout << seriesNumber[*lit][i] << " ";
		cout << endl;
		cout << endl << endl;
	}
}
...
void VolumeData_dicom::readSerie(int serieNumber)
{
	...
	for (lit = seriesImagesFilenames[serieNumber].begin(); lit != lend; ++lit)
	{
		...
		if (status.good())
		{
			...
			//cout << serieNumber << " " << posSlice << " " << posTime << " " << finalSeriesNumber << endl;
		
			seriesNumberMap[serieNumber][posSlice] = finalSeriesNumber;
		}
		else
		{
			ostringstream message;
	...
	for (; mit != mend; ++mit)
	{
		if (mit->second.size() != 1)
		{
			...
			for (int i=0; litPosSlice != lendPosSlice; ++litPosSlice, i++)
			{
				...
				printImage(serieNumber, *litPosSlice, 0, titre.str());
				userData[i].ptr = this;
...
void VolumeData_dicom_cardiac::choose_serie_ref()
{
	...
	while (!compatible)
	{
		...
		for (int n=0; lit != lend; ++lit, n++)
		{
			...
			printImage(*lit, sliceDisp, 0, nameDisp.str());
			userData[n].ptr = this;
			...
			int valIni = (int)maxVal;
			cv::createTrackbar("range", nameDisp.str(), &valIni, (int)maxVal, updateRange, &userData[n]);
		}
